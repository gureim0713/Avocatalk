...
public class Server_friendsList_controller {
	...
	public void friendsList_Query() {
		...
		ResultSet rs=null;
		String sql = "select m.name , m.member_id from friends fr, member m "
				+ "where fr.member_me_fk = "+user_pk+" and fr.member_myfriend_fk = m.member_id";
		try {
			...
			while (rs.next()) {
				friends_name.add(rs.getString("name"));
				friends_pk.add(rs.getString("member_id"));
			}
			if(rs != null) {
